import { describe, it, expect } from 'vitest';
import { SectorValuationEngine } from '../../src/domain/valuation/SectorValuationEngine';
import { MarketValuationSnapshot } from '../../src/domain/valuation/ValuationTypes';

describe('Phase 9 — DCF Scenarios & CAPM WACC Engine', () => {
  const baseMarketSnapshot: MarketValuationSnapshot = {
    currentPrice: 500.0,
    priceDate: '2024-03-31',
    marketDataTimestamp: new Date().toISOString(),
    currency: 'INR',
    shareCapital: {
      basicShares: 10.0,
      dilutedShares: 10.0,
      weightedAverageShares: 10.0,
      faceValue: 10.0,
      effectiveDate: '2024-03-31',
      corporateActionAdjustments: [],
      source: 'Exchange Filing',
      confidence: 95,
    },
    evBridge: {
      marketCapitalization: 5000.0,
      plusTotalDebt: 1000.0,
      plusPreferredEquity: 0,
      plusMinorityInterest: 0,
      lessCashAndEquivalents: 500.0,
      lessLiquidInvestments: 0,
      netDebt: 500.0,
      enterpriseValue: 5500.0,
      formulaDescription: 'Market Cap + Debt - Cash',
      accountingBasis: 'CONSOLIDATED',
      financialPeriod: 'FY24',
    },
    isStale: false,
    freshnessThresholdHours: 24,
    source: 'NSE',
    confidence: 95,
  };

  const facts: any[] = [
    { metricCode: 'REVENUE', value: 2000.0 },
    { metricCode: 'EBIT', value: 300.0 },
    { metricCode: 'EBITDA', value: 400.0 },
    { metricCode: 'PAT', value: 200.0 },
    { metricCode: 'TOTAL_EQUITY', value: 1500.0 },
  ];

  it('computes 3 distinct DCF scenarios (Bear, Base, Bull) with transparent WACC bridge', () => {
    const report = SectorValuationEngine.analyze(
      'proj_1',
      'TESTCORP',
      'OPERATING_INDUSTRIAL',
      'Manufacturing',
      baseMarketSnapshot,
      facts,
      [],
      null,
      null
    );

    const dcf = report.dcfModel;
    expect(dcf.waccBridge.wacc).toBeGreaterThan(10.0);
    expect(dcf.waccBridge.wacc).toBeLessThan(15.0);

    const bear = dcf.scenarios.BEAR;
    const base = dcf.scenarios.BASE;
    const bull = dcf.scenarios.BULL;

    expect(bear.revenueCagr).toBeLessThan(base.revenueCagr);
    expect(bull.revenueCagr).toBeGreaterThan(base.revenueCagr);

    expect(bear.valuePerShare).toBeLessThan(base.valuePerShare);
    expect(bull.valuePerShare).toBeGreaterThan(base.valuePerShare);

    // Each scenario has 5 projected cash flows
    expect(base.projectedCashFlows.length).toBe(5);
    expect(base.assumptions.length).toBeGreaterThanOrEqual(3);
  });
});
